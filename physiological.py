...
def getDataHelper(url: str, db) -> pd.DataFrame:
	...
	for i, participantData in enumerate(data):
		...
		try:
			dataObj['gameStartedAt'].append(participantData['timestamps']['gameStartedAt'])
		except:
			dataObj['gameStartedAt'].append('')
		...
		if participantData['timestamps']['resultsShownAt']:
			for j, ts in enumerate(participantData['timestamps']['resultsShownAt']):
				ts = datetime.strptime(ts, '%Y-%m-%dT%H:%M:%S.%fZ')
				dataObj[f'results{j}ShownAt'].append(str(ts))
